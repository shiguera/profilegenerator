...
public class M633_Reconstruct_2 {

	static Logger LOG = Logger.getLogger(M633_Reconstruct_2.class);
	...
	static XYVectorFunction originalVProfile;
	...
	public static void main(String[] args) {
		...
		originalVProfile = readOriginalVerticalProfile();
		originalVProfile = filter(originalVProfile,7);
		//XYVectorFunction gradeData = readGradeData();
		XYVectorFunction gradeData = createGradeData(originalVProfile);
	...
	private static XYVectorFunction filter(XYVectorFunction profile , int np) {
		XYVectorFunction recti = new XYVectorFunction();
		int first = 0;
		int last = 0;
		while(true) {
			last = first+np;
			...
			double[] r = profile.rectaMinimosCuadrados(first, last);
			for(int j=first; j<=last; j++) {
				double x = profile.getX(j);
				double[] xy = new double[]{x, r[0] + r[1]*x};
				recti.add(xy);
			}
			first = last+1;
		}
	...
	private static XYVectorFunction createGradeData(XYVectorFunction recti) {
		XYVectorFunction gradeData = new XYVectorFunction();
		gradeData.add(new double[]{0.0, 0.0});
		for(int i=1; i<recti.size(); i++) {
			double x = recti.getX(i);
			double incs = x - recti.getX(i-1);
			double incz = recti.getY(i) - recti.getY(i-1);
			double g = incz/incs;
			gradeData.add(new double[]{x,g});
		}
		double s0 = recti.getX(0);
		double g0 = gradeData.getY(1);
		gradeData.set(0, new double[]{s0, g0});
		return gradeData;
	}
	...
	private static void showProfile(XYVectorFunction profile) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
            public void run() {
        		charter = new Charter("M-633-2", "S", "Z");
	...
	private static XYVectorFunction readGradeData() {
		...
		URL url = ClassLoader.getSystemResource("Valdemanco_Cabanillas_2_SG.csv");
	...
	private static XYVectorFunction readOriginalVerticalProfile() {
		...
		URL url = ClassLoader.getSystemResource("Valdemanco_Cabanillas_2_SZ.csv");
		File file = new File(url.getPath());
		Assert.assertNotNull(file);
