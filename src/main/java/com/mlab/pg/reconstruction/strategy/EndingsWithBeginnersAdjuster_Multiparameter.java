...
public class EndingsWithBeginnersAdjuster_Multiparameter implements EndingsWithBeginnersAdjuster{
	...
	public EndingsWithBeginnersAdjuster_Multiparameter(XYVectorFunction originalGradePoints, ParameterIntervalArray parameters) {
		...
		while(changes) {
			...
			for(int i=1; i<process.size(); i++) {
				...
				if(g21 != g22) {
					...
					double th1 = parameterArray.getParameters(previous.getStartS()).getThresholdSlope();
					double th2 = parameterArray.getParameters(current.getStartS()).getThresholdSlope();					
					if (slope1<th1 && slope2<th2) {
						...
						newg3 = newg2;
					} else if (Math.abs(previous.getSlope()) < th1) {
	...
	private VerticalGradeProfile filterTwoGrades(VerticalGradeProfile profile) {
		...
		while(changes) {
			...
			for(int i=1; i<process.size(); i++) {
				...
				double th1 = parameterArray.getParameters(previous.getStartS()).getThresholdSlope();
				double th2 = parameterArray.getParameters(current.getStartS()).getThresholdSlope();					
				if(Math.abs(slope1)<th1 && Math.abs(slope2)<th2) {
